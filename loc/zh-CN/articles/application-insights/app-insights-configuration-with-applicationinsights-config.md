...
telemetry _modules_ 和 _initializers_ for automatically tracking telemetry from your application and its context. By 
adjusting the configuration file, you can enable or disable telemetry modules and initializers, and set parameters for 
some of them.

The configuration file is named `ApplicationInsights.config` 或 `ApplicationInsights.xml`, depending on the type of your
...
的 `Microsoft.ApplicationInsights` NuGet package provides the following telemetry modules in the 
...
```
的 `DiagnosticsTelemetryModule` reports errors in the Application Insights instrumenation code itself. For example, 
...
的 `Microsoft.ApplicationInsights.DependencyCollector` NuGet package provides the following telemetry modules in 
`ApplicationInsights.config`.
...
```
的 `DependencyTrackingTelemetryModule` tracks telemetry about calls to external dependencies made by your application, 
...
的 `Microsoft.ApplicationInsights.Web` NuGet package provides the following telemetry initializers and modules in the
`ApplicationInsights.config`.
...
* `SyntheticTelemetryInitializer` updates the `User`, `Session` 和 `Operation` contexts properties of all telemetry 
...
* `ClientIpHeaderTelemetryInitializer` updates `Ip` 属性的 `Location` context of all telemetry items based on the 
`X-Forwarded-For` HTTP header of the request.
* `UserAgentTelemetryInitializer` updates the `UserAgent` 属性的 `User` context of all telemetry items based 
on the `User-Agent` HTTP header of the request.
* `OperationNameTelemetryInitializer` updates the `Name` 属性的 `RequestTelemetry` and the `Name` property of 
...
* `UserTelemetryInitializer` updates the `Id` 和 `AcquisitionDate` properties of `User` context for all telemetry items
...
* `SessionTelemetryInitializer` updates the `Id` 属性的 `Session` context for all telemetry items with value
extracted from the `ai_session` cookie generated by the ApplicationInsights JavaScript instrumentation code running in the
user's browser. 
* `AzureRoleEnvironmentTelemetryInitializer` updates the `RoleName` 和 `RoleInstance` properties of the `Device` context
for all telemetry items with information extracted from the Azure runtime environment.
* `DomainNameRoleInstanceTelemetryInitializer` updates the `RoleInstance` 属性的 `Device` context for all
telemetry items with the domain name of the computer where the web application is running.
* `BuildInfoConfigComponentVersionTelemetryInitializer` updates the `Version` 属性的 `Component` context for 
...
的 `Microsoft.ApplicationInsights.PerfCounterCollector` NuGet package adds the following telemetry modules to the `ApplicationInsights.config` by default.
...
`PerformanceCounter` must be either `\CategoryName(InstanceName)\CounterName` 或 `\CategoryName\CounterName`
