...
* Le **API gateway** is the endpoint that:
...
* Le **publisher portal** is the administrative interface where you set-up your API program:
...
* Le **developer portal** serves as the main web presence for developers where they can:
...
The first step in working with API Management is to create a service instance. Log in to the [Portail de gestion][] and click **New**, **App Services**, **Gestion de l'API**, **Create**.
...
Choose the desired **Subscription** et **Region** for your service instance. After making your selections, click the next button.
...
>For guidance on creating an API and manually adding operations, see [How to create APIs](api-management-howto-create-apis.md) et [How to add operations to an API](api-management-howto-add-operations.md).
...
To import the calculator API, click **APIs** de la **Gestion de l'API** menu on the left, and then click **Import API**.
...
After an operation is invoked, the developer portal displays the **Response status**, le **Response headers**, and any **Response content**.
...
For detailed metrics and reports, click **Analytics** de la **Gestion de l'API** menu on the left.

![Vue d'ensemble][api-management-analytics-overview]

Le **Analytics** section has the following four tabs.

-	**En un clin d'oeil** provides overall usage and health metrics as well as the top developers, top products, top APIs, and top operations.
-	**Son utilisation** provides in-depth look at API calls and bandwidth including a geographical representation.
-	**Santé** focuses on status codes, cache success rates, response times, and API and service response times.
-	**Activité** provides reports that drill down on the specific activity by developer, product, API, and operation.
