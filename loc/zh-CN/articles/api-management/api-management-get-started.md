...
* 的 **API gateway** is the endpoint that:
...
* 的 **publisher portal** is the administrative interface where you set-up your API program:
...
* 的 **developer portal** serves as the main web presence for developers where they can:
...
The first step in working with API Management is to create a service instance. Log in to the [管理门户][] and click **New**, **App Services**, **API 管理**, **Create**.
...
Choose the desired **Subscription** 和 **Region** for your service instance. After making your selections, click the next button.
...
>For guidance on creating an API and manually adding operations, see [How to create APIs](api-management-howto-create-apis.md) 和 [How to add operations to an API](api-management-howto-add-operations.md).
...
To import the calculator API, click **APIs** 从 **API 管理** menu on the left, and then click **Import API**.
...
After an operation is invoked, the developer portal displays the **Response status**， **Response headers**, and any **Response content**.
...
For detailed metrics and reports, click **Analytics** 从 **API 管理** menu on the left.

![概述][api-management-analytics-overview]

的 **Analytics** section has the following four tabs.

-	**在一瞥** provides overall usage and health metrics as well as the top developers, top products, top APIs, and top operations.
-	**使用** provides in-depth look at API calls and bandwidth including a geographical representation.
-	**健康** focuses on status codes, cache success rates, response times, and API and service response times.
-	**活动** provides reports that drill down on the specific activity by developer, product, API, and operation.
