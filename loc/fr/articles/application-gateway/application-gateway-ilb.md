...
- Le *FrontendIPConfigurations* element describes the ILB details relevant for configuring Application Gateway with an ILB. 

- The Frontend IP *Type* should be set to 'Private'

- Le *StaticIPAddress* should be set to the desired internal IP on which the gateway will receive traffic. Note that the  *StaticIPAddress* element is optional. If not set, an available internal IP from the deployed subnet is chosen. 
