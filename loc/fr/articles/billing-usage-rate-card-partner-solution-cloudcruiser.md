...
# Nuage de Cruiser et Azure de Microsoft API intégration de facturation 

Cet article décrit comment les renseignements recueillis sur les nouvelles Microsoft Azure facturation API peuvent être utilisées en nuage Cruiser pour l'analyse et la simulation de coût de flux de travail.

## Tarifs Azur API
L'API de tarifs renseigne les taux d'Azure. Après l'authentification avec les informations d'identification appropriées, vous pouvez interroger l'API afin d'obtenir des métadonnées sur les services disponibles sur Azure, ainsi que les tarifs liés à votre identifiant d'offrir. 

Voici une réponse de l'échantillon de l'API indiquant les prix pour le A0 instance (Windows) :
...
## Interface du croiseur d'Azur tarifs API des nuages
Cruiser de nuages peut exploiter les informations tarifs API de différentes manières. Pour cet article, nous allons montrer comment il peut être utilisé pour faire IaaS coût d'analyse et simulation de charge de travail.

Pour illustrer ce cas d'utilisation, Imaginez une charge de travail de plusieurs instances en cours d'exécution sur Microsoft Azure Pack (WAP). Le but est de simuler cette même charge de travail sur Azure et estimer les coûts de ces migrations. Afin de créer cette simulation, il existe deux principales tâches à accomplir :

1. **Importer et traiter les informations de service prélevées par l'API de tarifs** -Cette tâche est également effectuée sur les classeurs, où l'extrait de l'API de tarifs est transformé et publié à un nouveau forfait. Ce nouveau forfait serviront sur les simulations pour estimer le prix d'Azur.

2. **Normaliser les services WAP et services Azure pour IaaS** -Par défaut, les services WAP sont basées sur les ressources individuelles (CPU, mémoire, taille du disque, etc.) tout en Azure services sont basés sur la taille de l'instance (A0, A1, A2, etc..). Cette première tâche peut être réalisée par ETL moteur du nuage croiseur, appelé classeurs, où ces ressources peuvent être regroupés sur des tailles d'instance, analogues aux services de l'instance Azure.

## Importer des données de l'API de tarifs

Nuage Cruiser classeurs fournissent un moyen automatisé de recueillir et traiter l'information de l'API de tarifs.  Classeurs ETL (extract-transform-load) vous permettent de configurer la collecte, de transformation et publication de données dans la base de nuage Cruiser.

Chaque classeur peut avoir une ou plusieurs collections. Cela permet de corréler l'information provenant de différentes sources pour compléter ou augmenter les données d'utilisation. Dans les deux captures d'écran ci-dessous, nous montrant la création d'un nouveau *collection* dans un classeur existant et importer des informations dans le *collection* depuis les tarifs API :
