...
>[AZURBLAU. HINWEIS] If you already have a product configured and want to use it for this tutorial, you can jump ahead to [Configure call rate limit and quota policies][] and follow the tutorial from there using your product in place of the **Free Trial** product.
...
Products in API Management can be **Open** oder **Protected**. Protected products must be subscribed to before they can be used, while open products can be used without a subscription. Ensure that **Require subscription** is checked to create a protected product that requires a subscription. This is the default setting.
...
Click **Products** aus der **API-Management** menu on the left, and click **Free Trial** to configure the product.
...
Rate limits and quotas are configured in the policy editor. Click **Richtlinien** under the **API-Management** menu on the left, and select **Free Trial** aus der **Policy Scope Product** drop-down.
...
To insert policies, position the cursor into either the **inbound** oder **outbound** section of the policy template. Rate limit and quota policies are inbound policies, so position the cursor in the inbound element.

![Policy editor][api-management-policy-editor-inbound]

The two policies we are adding in this tutorial are the [Limit call rate][] und [Set usage quota][] policies.
...
Because this policy is also intended to be at the product level, delete the **API** und **operation** name elements, as shown in the following example.
...
Now that the the APIs are added and the policies configured, the product is ready to be used by developers. Before the product can be used by developers, it must be published. Click **Products** aus der **API-Management** menu on the left, and click **Free Trial** to configure the product.
...
>[AZURBLAU. HINWEIS] In this tutorial, multiple simultaneous subscriptions are not enabled for the **Free Trial** product. If they were, you would be prompted to name the subscription, as shown in the following example.
...
>[AZURBLAU. HINWEIS] If you have multiple subscriptions be sure to select the key for **Free Trial**, or else the policies that were configured in the previous steps won't be in effect.
...
Die **Response Headers** and the **Response content** indicate the remaining interval before retries will be successful.
