...
>[AZURE. REMARQUE] The process for backing up and restoring an API Management service instance for disaster recovery can also be used for replicating API Management service instances for scenarios such as staging.
...
Select **Windows** **Azur Service Management API** and click the checkbox to add the application.

![Add permissions][api-management-aad-permissions]

Click **Delegated Permissions** beside the newly added **Windows** **Azur Service Management API** application, check the box for **Access Azure Service Management (preview)**, and click **Save**.
...
Replace `{tentand id}`, `{application id}`, et `{redirect uri}` using the following instructions.
...
Replace `{application id}` et `{redirect uri}` using the **Client Id** and  the URL from the **Redirect Uris** section from your Azure Active Directory application's **Configure** tab. 
