...
Select **Windows** **Azure のサービス管理 API** and click the checkbox to add the application.

![Add permissions][api-management-aad-permissions]

Click **Delegated Permissions** beside the newly added **Windows** **Azure のサービス管理 API** application, check the box for **Access Azure Service Management (preview)**, and click **Save**.
...
Replace `{tentand id}`, `{application id}`、と `{redirect uri}` using the following instructions.
...
Replace `{application id}` と `{redirect uri}` using the **Client Id** and  the URL from the **Redirect Uris** section from your Azure Active Directory application's **Configure** tab. 
...
>[AZURE。重要です] **The SKU** of the service being restored into **must match** the SKU of the backed up service being restored.
>
