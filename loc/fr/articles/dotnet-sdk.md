...
# Quel est le SDK Azure pour .NET ?

## Vue d'ensemble

Le SDK Azure pour .NET est un ensemble d'outils Visual Studio, des outils de ligne de commande, fichiers binaires d'exécution et les bibliothèques clientes qui vous aident à développent, tester et déploiement des applications qui s'exécutent dans Azure. Cet article expose en détail ce que vous obtenez lorsque vous installez le SDK. Vous pouvez télécharger le SDK de la [Page de téléchargement d'Azur](/downloads/). 

Le SDK Azure pour .NET comprend également [bibliothèques clientes pour consommer des services Azure](http://go.microsoft.com/fwlink/?LinkId=510472). Ces bibliothèques sont installés séparément à l'aide de NuGet.

##<a id="included"></a>Ce qui est inclus dans le SDK Azure pour .NET

Le SDK Azure pour .NET installe les produits suivants :

- [Visual Studio Express pour le Web](#vwd)
- [Microsoft ASP.NET et Web Tools pour Visual Studio](#wte)
- [Microsoft Azure Tools pour Microsoft Visual Studio](#tools)
- [Outils de création de Microsoft Azure](#auth)
- [Émulateur de Microsoft Azure](#emulator)
...
- [HDInsight Tools pour Visual Studio](#hdinsight) et [Microsoft Hive ODBC Driver](#hdinsight)
- [Microsoft Azure Mobile App SDK V1.0](#mobile)
- [Microsoft Azure PowerShell](#ps)

###<a id="vwd"></a>Visual Studio Express pour le Web

If you don't have Visual Studio on your computer, the SDK will install [Visual Studio Express pour le Web](http://www.visualstudio.com/products/visual-studio-express-vs.aspx). 
 
###<a id="wte"></a>Microsoft ASP.NET et Web Tools pour Visual Studio
...
>[AZURE. REMARQUE] You don't have to install the Azure SDK for .NET to use these features; they are also included in Visual Studio Updates. 

###<a id="tools"></a>Microsoft Azure Tools pour Microsoft Visual Studio
...
* View and manage [cloud services](http://msdn.microsoft.com/library/ff683675.aspx), [virtual machines](http://msdn.microsoft.com/library/jj131259.aspx), et [Bus de service](http://msdn.microsoft.com/library/jj149828.aspx) in Server Explorer. 
...
* Le [CSPack command-line tool](http://msdn.microsoft.com/library/gg432988.aspx) for creating deployment packages.
...
###<a id="emulator"></a>Émulateur de Microsoft Azure

Le [Azure Emulator](http://msdn.microsoft.com/library/dn339018.aspx) simulates the cloud service environment so that you can test cloud service projects locally on your computer before you deploy them to Azure.

###<a id="stgemulator"></a>Émulateur de stockage Microsoft Azure

Le [Azure Storage Emulator](http://msdn.microsoft.com/library/hh403989.aspx) uses a SQL Server instance and the local file system to simulate Azure Storage (queues, tables, blobs), so that you can test locally. 
...
Each item shown in this article under [Ce qui est inclus dans le SDK Azure pour .NET](#included) is a separate program in the list of installed programs in Windows Control Panel **Programs and Features**.  There is no way to uninstall them as a group; you have to uninstall each program individually.
...
To download the current Azure SDK for .NET or a client library, see the [Page de téléchargement d'Azur](/downloads/).
