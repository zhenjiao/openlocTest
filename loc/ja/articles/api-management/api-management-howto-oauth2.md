...
>[AZURE。メモ] For more information on configuring OAuth 2.0 using Azure Active Directory, see the [WebApp-GraphAPI-DotNet][] sample.
...
>[AZURE。メモ] If you have not yet created an API Management service instance, see [Create an API Management service instance][] in the [Get started with Azure API Management][] tutorial.

Click **セキュリティ** から、 **API の管理** menu on the left, click **OAuth 2.0**, and then click **Add authorization server**.
...
Enter a name and an optional description in the **Name** と **Description** fields. 

>[AZURE。メモ] These fields are used to identify the OAuth 2.0 authorization server within the current API Management service instance and their values do not come from the OAuth 2.0 server.
...
The next section of the form contains the **Authorization code grant types**, **Authorization endpoint URL**、と **Authorization request method** settings.
...
、 **Authorization request method** specifies how the authorization request is sent to the OAuth 2.0 server. By default **GET** is selected.

The next section is where the **Token endpoint URL**, **Client authentication methods**, **Access token sending method**、と **Default scope** are specified.
...
The default setting for **Client authentication methods** は **Basic**、と  **Access token sending method** は **Authorization header**. These values are configured on this section of the form, along with the **Default scope**.

、 **Client credentials** section contains the **Client ID** と **Client secret**, which are obtained during the creation and configuration process of your OAuth 2.0 server. Once the **Client ID** と **Client secret** are specified, the **redirect_uri** for the **authorization code** is generated. This URI is used to configure the reply URL in your OAuth 2.0 server configuration.
...
Click **APIs** から、 **API の管理** menu on the left, click the name of the desired API, click **セキュリティ**, and then check the box for **OAuth 2.0**.
...
>[AZURE。メモ] If you have only one API configured or visible to your account, then clicking APIs takes you directly to the operations for that API.
...
>[AZURE。メモ] If you have pop-ups disabled you will be prompted to enable them by the browser. After you enable them, select **Authorization code** again and the sign-in form will be displayed.
