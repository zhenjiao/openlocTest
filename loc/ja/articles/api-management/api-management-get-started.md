...
* 、 **publisher portal** is the administrative interface where you set-up your API program:
...
The first step in working with API Management is to create a service instance. Log in to the [管理ポータル][] and click **New**, **App Services**, **API の管理**, **Create**.
...
Choose the desired **Subscription** と **Region** for your service instance. After making your selections, click the next button.
...
>For guidance on creating an API and manually adding operations, see [How to create APIs](api-management-howto-create-apis.md) と [How to add operations to an API](api-management-howto-add-operations.md).
...
To import the calculator API, click **APIs** から、 **API の管理** menu on the left, and then click **Import API**.
...
For detailed metrics and reports, click **Analytics** から、 **API の管理** menu on the left.

![概要][api-management-analytics-overview]

、 **Analytics** section has the following four tabs.

-	**一目で** provides overall usage and health metrics as well as the top developers, top products, top APIs, and top operations.
-	**使い方** provides in-depth look at API calls and bandwidth including a geographical representation.
-	**健康** focuses on status codes, cache success rates, response times, and API and service response times.
-	**アクティビティ** provides reports that drill down on the specific activity by developer, product, API, and operation.
