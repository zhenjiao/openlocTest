...
![リソースおよびリソース グループのブレードにタグ部分](./media/resource-group-using-tags/rgblade.png)
...
Tagging is only available for resources and resource groups available from [Resource Manager](http://msdn.microsoft.com/library/azure/dn790568.aspx), so the next thing we need to do is switch to use Resource Manager. For more information, see [Azure リソース マネージャーと紺碧の PowerShell を使用](powershell-azure-resource-manager.md).

    Switch-AzureMode AzureResourceManager

Tags exist directly on resources and resource groups, so to see what tags are already applied, we can simply get a resource or resource group with `Get-AzureResource` または `Get-AzureResourceGroup`, respectively. Let's start with a resource group.
...
The process is the same for resources, except you'll use the `Get-AzureResource` と `Set-AzureResource` cmdlets. To get resources or resource groups with a specific tag, use `Get-AzureResource` または `Get-AzureResourceGroup` cmdlet with the `-Tag` parameter.
...
使用、 `New-AzureTag` cmdlet to add new tags to the taxonomy. These tags will be included in the autocomplete even though they haven't been applied to any resources or resource groups, yet. To remove a tag name/value, first remove the tag from any resources it may be used with and then use the `Remove-AzureTag` cmdlet to remove it from the taxonomy.
...
the [Azure accounts portal](https://account.windowsazure.com/) または [EA portal](https://ea.azure.com).
...
- [Azure リソース マネージャーと紺碧の PowerShell を使用](./powershell-azure-resource-manager.md)
