...
* **Azure エミュレーター (Windows のみ)**. The compute and storage emulators are local emulators of Cloud Services and Data Management Services that allow you to test an application locally. The Azure Emulators run on Windows only.
...
* From a reputable distro such as [www.continuum.io][], [www.enthought.com][] または [www.activestate.com][]
* Build from source!
...
、 [Python Package Index][] (PyPI) has a rich selection of Python libraries.  If you chose to install a Distro, you'll already have most of the interesting bits for a variety of scenarios from web development to Technical Computing.


### Visual Studio の Python ツール

[Visual Studio の Python ツール][] (PTVS) is a free/OSS plugin from Microsoft which turns VS into a full-fledged Python IDE:
...
PTVS also makes it easy to deploy to Microsoft Azure, with support for deployment to [クラウド サービス][] と [Websites][].
...
For more information on developing and publishing Azure Websites, see the tutorials for [Creating Websites with Django][], [Creating Websites with Bottle][] と [Creating Websites with Flask][]. For more general information on using any WSGI compliant framework, see [Configuring Python with Azure Websites][].
