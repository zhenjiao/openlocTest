...
Nutzung der `Get-AzureApplicationGateway` cmdlet to check the status of gateway. If *Start-AzureApplicationGateway* succeeded in the previous step, the State should be *Ausführen*, and the Vip and DnsName should have valid entries. 
...
1. Nutzung der `Stop-AzureApplicationGateway` cmdlet to stop the gateway. 
2. Nutzung der `Remove-AzureApplicationGateway` cmdlet to remove the gateway.
