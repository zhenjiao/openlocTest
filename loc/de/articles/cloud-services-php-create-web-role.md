...
#How Erstellen von PHP-Web und Worker-Rollen

## Übersicht

Diese Anleitung zeigt Ihnen, wie man PHP Web oder Worker Rollen in einer Windows-Entwicklungsumgebung erstellen, wählen eine bestimmte Version von PHP aus der "eingebauten" Versionen verfügbar, die PHP-Konfiguration ändern, Erweiterungen aktivieren und schließlich auf Azure bereitstellen. Es beschreibt auch konfigurieren eine Web- oder Arbeitnehmer Rolle um eine PHP-Laufzeit (mit benutzerdefinierte Konfiguration und Erweiterungen) zu verwenden, die Sie bereitstellen.

## Welche spielen PHP Web und Worker Rolle?
Azurblau bietet drei Modelle zum Ausführen von Anwendungen zu berechnen: [Azurblaue Websites][execution model-web sites], [Azurblaue virtueller Maschinen][execution model-vms], und [Azure Cloud Services][execution model-cloud services]. Alle drei Modelle unterstützen PHP. Cloud-Services, die Web und Worker Rollen umfassen, bieten *Platform as a Service (PaaS)*. Innerhalb einer Cloud-Service bietet eine Webserverrolle ein dedizierten Webservers für Internet Information Services (IIS) von Host-Front-End-Web-Anwendungen, während eine Arbeiter-Rolle asynchrone, lang andauernde oder unbefristete Aufgaben unabhängig von Benutzerinteraktionen oder Eingabe ausgeführt werden kann.

Weitere Informationen finden Sie unter [Was ist ein Cloud-Service?].

## Laden Sie das Azure SDK für PHP

Die [Azure SDK für PHP] besteht aus mehreren Komponenten. In diesem Artikel werden zwei davon verwenden: PowerShell Azure und die Azure-Emulatoren. Diese beiden Komponenten können über den Microsoft Web Platform Installer hier installiert werden: [Install Azure PowerShell and the Azure Emulators][install ps and emulators].

## How to: Create a Cloud Services project

The first step in creating a PHP web or worker role is to create an Azure Service project. an Azure Service project serves as a logical container for web and worker roles, and contains the project's [service definition (.csdef)] und [service configuration (.cscfg)] files.
...
> [AZURBLAU. HINWEIS] Die `roleName` parameter is optional. If it is omitted, the role name will be automatically generated. The first web role created will be `WebRole1`, the second `WebRole2`, and so on. The first worker role created will be `WorkerRole1`, the second `WorkerRole2`, and so on.
...
> [AZURBLAU. HINWEIS] More PHP versions may be available in the future, and the available versions may change.
...
> [AZURBLAU. HINWEIS] Any settings that you don't explicity set in the `php.ini` file that you provide will automatically be set to their default values. However, keep in mind that you can add a complete `php.ini` file.
...
1. Create an Azure Service project and add a PHP web role as described in the [How to: Create a cloud services project](#how-to-create-a-cloud-services-project) und [How to: Add PHP web or worker roles](#how-to-add-php-web-or-worker-roles) sections above.
...
> [AZURBLAU. HINWEIS] Die `download.ps1` script (in the `bin` folder of the web role's root directory) can be deleted after following the steps described above for using your own PHP runtime.
...
1. Create an Azure Service project and add a PHP worker role as described in the [How to: Create a cloud services project](#how-to-create-a-cloud-services-project) und [How to: Add PHP web or worker roles](#how-to-add-php-web-or-worker-roles) sections above.
...
To publish your application, you need to first import your publish settings  with the **Import-PublishSettingsFile** cmdlet, then you can publish your application with the **Publish-AzureServiceProject** cmdlet. Details on using each of these cmdlets can be found in [How to: Import publish settings] und [How to: Deploy a cloud service to Azure] respectively.
