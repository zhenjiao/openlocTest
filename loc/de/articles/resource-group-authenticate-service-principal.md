...
If you do not have Azure PowerShell installed, see [Zum Installieren und Konfigurieren von Azure PowerShell](./powershell-install-configure.md).
...
4. Retrieve the subscription in which the role assignment was created. This subscription will be used later to get the **TenantId** of the tenant that the service principal's role assignment resides in.
     ...
     If you created the role assignment in a subscription other than the currently selected subscription, you can specify the **SubscriptoinId** oder **SubscriptionName** parameters to retrive a different subscription.
...
5. Create a new **PSCredential** object which contains your credentials by running the **Get-Credential** command.
     ...
     For the user name, use the **ApplicationId** oder **IdentifierUris** that you used when creating the application. For the password, use the one you specified when creating the account.
