...
The lock level identifies the type of enforcement for the policy, which presently has two values – **CanNotDelete** と **ReadOnly**. The scope is expressed as a URI and can be either a 
...
[Role-based access control in the preview portal](role-based-access-control-configure.md) と [Managing and Auditing Access to Resources](resource-group-rbac.md).
...
For lock-level, specify either **CanNotDelete** または **ReadOnly**.
