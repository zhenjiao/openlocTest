...
An Application Insights resource is a place in Microsoft Azure where telemetry data from your app is analyzed and displayed. You might already have set up one to display data from your app's server side in [ASP.NET][greenbrown] ou [J2EE][java]. If not, you might want to do that now.
...
If you instrument both the client and server ([ASP.NET][greenbrown] ou [J2EE][java]), the SDKs will propagate the session id between client and server, so that events on both sides can be correlated.
