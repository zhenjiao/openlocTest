...
# Comment utiliser le Service gestion de PHP

Ce guide vous montrera comment effectuer par programme des tâches courantes de gestion de service de PHP. Le [ServiceManagementRestProxy] classe dans le [Azure SDK pour PHP][download-SDK-PHP] prend en charge l'accès par programme à une grande partie de la fonctionnalité de service liées à la gestion qui est disponible dans la [portail de gestion][management-portal] (tels que **création, mise à jour et suppression de services cloud, des déploiements, des services de stockage et groupes d'affinité**). Cette fonctionnalité peut être utile dans les applications nécessitant un accès par programme à la gestion des services de construction.

## Ce qui est gestion des services
L'API de gestion de Service fournit l'accès à une grande partie de la fonctionnalité de gestion de service disponible via le [portail de gestion][management-portal]. Le SDK Azure pour PHP permet de gérer vos services cloud, comptes de stockage et des groupes d'affinité.

Pour utiliser l'API de gestion de Service, vous devrez [créer un compte Azur][win-azure-account].

## Concepts
Le SDK Azure wraps PHP le [Azur Service Management API][svc-mgmt-rest-api], which is a REST API. All API operations are performed over SSL and mutually authenticated using X.509 v3 certificates. The management service may be accessed from within a service running in Azure, or directly over the Internet from any application that can send an HTTPS request and receive an HTTPS response.
...
After you have created these files, you will need to upload the `.cer` file to Azure via the [portail de gestion][management-portal], and you will need to make note of where you saved the `.pem` file.
...
In the example above, `$serviceManagementRestProxy` is a [ServiceManagementRestProxy] objet. Le **ServiceManagementRestProxy** class is the primary class used to manage Azure services.
...
When you create an application and run it in Azure, the code and configuration together are called an Azure [cloud service]  - known as a *hosted service* in earlier Azure releases. The **createHostedServices** method allows you to create a new hosted service by providing a hosted service name (which must be unique in Azure), a label (the base 64-endcoded hosted service name), and a **CreateServiceOptions** objet. Le [CreateServiceOptions] object allows you to set the location *ou* the affinity group for your service.
...
After you have created a cloud service, you can deploy your code to the service with the [createDeployment](#CreateDeployment) méthode.
...
Note in the example above that the status of the **createDeployment** operation can be retrieved by passing the result returned by **createDeployment** to the **getOperationStatus** méthode.
...
A deployment can be updated by using the **changeDeploymentConfiguration** method or the **updateDeploymentStatus** méthode.

Le **changeDeploymentConfiguration** method allows you to upload a new service configuration (`.cscfg`) file, which will change any of several service settings (including the number of instances in a deployment). For more information, see [Azure Service Configuration Schema (.cscfg)]. The following example demonstrates how to upload a new service configuration file:
...
Note in the example above that the status of the **changeDeploymentConfiguration** operation can be retrieved by passing the result returned by **changeDeploymentConfiguration** to the **getOperationStatus** méthode.

Le **updateDeploymentStatus** method allows you to set a deployment status to RUNNING or SUSPENDED. The following example demonstrates how to set the status to RUNNING for a deployment in the production slot of a hosted service called `myhostedservice`:
...
A [storage service] gives you access to Azure [Blobs][azure-blobs], [Tables][azure-tables], et [Queues][azure-queues]. To create a storage service, you need a name for the service (between 3 and 24 lowercase characters and unique within Azure), a label (a base-64 encoded name for the service, up to 100 characters), and either a location or an affinity group. Providing a description for the service is optional. The location, affinity group, and description are set in a [CreateServiceOptions] object, which is passed to the **createStorageService** method. The following example shows how to create a storage service by specifying a location. If you want to use an affinity group, you have to create an affinity group first (see [How to: Create an affinity group](#CreateAffinityGroup)) and set it with the **CreateServiceOptions->setAffinityGroup** méthode.
...
Note in the example above that the status of the **createStorageService** operation can be retrieved by passing the result returned by **createStorageService** to the **getOperationStatus** méthode.  
