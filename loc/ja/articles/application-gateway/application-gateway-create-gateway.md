...
使用、 `Get-AzureApplicationGateway` cmdlet to check the status of gateway. If *Start-AzureApplicationGateway* succeeded in the previous step, the State should be *実行しています。*, and the Vip and DnsName should have valid entries. 
...
1. 使用、 `Stop-AzureApplicationGateway` cmdlet to stop the gateway. 
2. 使用、 `Remove-AzureApplicationGateway` cmdlet to remove the gateway.
