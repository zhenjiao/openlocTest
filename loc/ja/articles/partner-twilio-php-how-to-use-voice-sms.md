...
# Twilio を音声および SMS 機能を PHP で使用する方法
...
**Twilio 声** 携帯電話の通話を送受信するアプリケーションをできます。 **Twilio SMS** テキスト メッセージを送受信するアプリケーションを有効にします。 **Twilio Client** allows you to make VoIP calls from any phone, tablet, or browser and supports WebRTC.

## <a id="Pricing"></a>Twilio 価格や特別オファー

紺碧のお客様に、 [特別オファー](http://www.twilio.com/azure): Twilio クレジット Twilio アカウントをアップグレードするときの無料 $10。この Twilio クレジットは Twilio 使用 ($10 のクレジットとして 1,000 の SMS メッセージを送信または受信電話番号およびメッセージまたは呼び出し先の場所によっては、受信最大 1000年の声分に相当) に適用できます。この Twilio クレジットし、で開始。 [http://ahoy.twilio.com/azure](http://ahoy.twilio.com/azure).

Twilio は、従量課金制のサービスです。セットアップ料金がない、いつでも自分のアカウントを閉じることができます。詳細を見つけることができます。 [Twilio 価格][twilio_pricing].
...
Detailed instructions for building and deploying a Twilio/PHP application to Azure are available at [Azure 上の PHP アプリケーションの Twilio を使用して電話をかける方法][howto_phonecall_php].
...
詳細については、次を参照してください。 [https://github.com/twilio/twilio-php/blob/master/README.md][twilio_github_readme].
...
For additional information about using Twilio in Azure with PHP, see [Azure 上の PHP アプリケーションの Twilio を使用して電話をかける方法][howto_phonecall_php].
