...
>[AZURE. REMARQUE] For more information on configuring OAuth 2.0 using Azure Active Directory, see the [WebApp-GraphAPI-DotNet][] sample.
...
>[AZURE. REMARQUE] If you have not yet created an API Management service instance, see [Create an API Management service instance][] in the [Get started with Azure API Management][] tutorial.

Click **Sécurité** de la **Gestion de l'API** menu on the left, click **OAuth 2.0**, and then click **Add authorization server**.
...
Enter a name and an optional description in the **Name** et **Description** fields. 

>[AZURE. REMARQUE] These fields are used to identify the OAuth 2.0 authorization server within the current API Management service instance and their values do not come from the OAuth 2.0 server.
...
The next section of the form contains the **Authorization code grant types**, **Authorization endpoint URL**, et **Authorization request method** settings.
...
Le **Authorization request method** specifies how the authorization request is sent to the OAuth 2.0 server. By default **GET** is selected.

The next section is where the **Token endpoint URL**, **Client authentication methods**, **Access token sending method**, et **Default scope** are specified.
...
The default setting for **Client authentication methods** est **Basic**, et  **Access token sending method** est **Authorization header**. These values are configured on this section of the form, along with the **Default scope**.

Le **Client credentials** section contains the **Client ID** et **Client secret**, which are obtained during the creation and configuration process of your OAuth 2.0 server. Once the **Client ID** et **Client secret** are specified, the **redirect_uri** for the **authorization code** is generated. This URI is used to configure the reply URL in your OAuth 2.0 server configuration.

![New server][api-management-oauth2-server-4]

If **Authorization code grant types** is set to **Resource owner password**, le **Resource owner password credentials** section is used to specify those credentials; otherwise you can leave it blank.
...
Click **APIs** de la **Gestion de l'API** menu on the left, click the name of the desired API, click **Sécurité**, and then check the box for **OAuth 2.0**.
...
>[AZURE. REMARQUE] If you have only one API configured or visible to your account, then clicking APIs takes you directly to the operations for that API.
...
>[AZURE. REMARQUE] If you have pop-ups disabled you will be prompted to enable them by the browser. After you enable them, select **Authorization code** again and the sign-in form will be displayed.
